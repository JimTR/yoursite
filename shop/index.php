...
if($Auth->loggedIn()) 
	...
	if ($settings['forumtabs'] == true)
    {
		...
		foreach ($root as $row)
				{
					...
					$tabs->load($site->settings['template_path']."/shop/tab.html",false); //dont show this templates remarks  
					...
					$sql = "SELECT * , 
							...
							if ($ul == "1") {
				                ...
				                $template->load($site->settings['template_path']."/shop/forum_group.html",COMMENT);
			...
			{
				 $sql = "SELECT topic_id , topic_subject, categories.cat_id, categories.cat_name,categories.icon,
				 ...
				 $subtemplate->load($site->settings['template_path']."/shop/forum_row.html", COMMENT); // load the row template
					...
					if ($catrow['isgroup'] == 1)
						{
							...
							$subtemplate->replace ("icon",$catrow['icon']);
			                ...
			                $xx ="";
			//echo $totalthreads."-";
			                    $tab_entry['tab_title'] .= "Items ".$totalthreads; 
		...
		$template->load ($site->settings['template_path'].'/shop/tabindex.html', COMMENT);
	}
	else 
	{
		// do a classic view
