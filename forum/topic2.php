...
if($Auth->loggedIn()) 
	...
	$page['navi'] ='<div style="padding:0.5%;"><a style="color:#FFFFFF" href="index.php?activetab='.$activetab.'">Forum</a>';
	$page['newthread'] = 0;
	$page['usertitle'] = "";
	$page['pmnew'] = 0;
...
else
{
	...
	else
	{
			...
			//display post data
			        ...
			        $page['navi'].='-><a style="color:#fff" href="category.php?id='.$row['topic_cat'].'&activetab='.$activetab.'">'.$row['cat_name'].'</a>->'.$row['topic_subject'];
			...
			else
			{
				...
				foreach ($post_data as $posts_row)
				{
					...
					if($Auth->loggedIn()){
						...
						$post_info['reply'] = setbutton("Reply","location.href='#reply1'",false);
						...
						if ($Auth->level=== 'admin') {$post_info['iplogged']= 'IP Logged - '. $posts_row['post_ip'];}
						}
...
function setbutton($text,$location,$type) {
